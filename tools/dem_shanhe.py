#!/usr/bin/env python3
"""《山河之限》高程数据管线：下载 AWS Terrarium DEM 瓦片(z6)、拼接、裁剪、量化编码。

产物: data/dem_shanhe/height.png + meta.json
编码: h = (R*256 + G) * 4 - 800 米（海裁剪到 -800m，陆地到 4000m，4m 量化）
...
OUT = os.path.join(ROOT, "data/dem_shanhe"); os.makedirs(OUT, exist_ok=True)

Z = 6
X0, X1 = 50, 55
Y0, Y1 = 22, 26
LON_MIN, LON_MAX = 104.0, 134.0
LAT_MIN, LAT_MAX = 29.0, 46.0
H_MIN, H_MAX, QUANT = -800, 4000, 4
WORLD = 256 * (2 ** Z)
...
meta = {"width": W, "height": H,
        ...
        "encode": f"h = (R*256+G)*{QUANT} - {-H_MIN} (m), sea clamped at {H_MIN}, land at {H_MAX}"}
